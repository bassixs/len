...
export function initCategoryProducts() {
  ...
  fetch(url)
    ...
    .then(allProducts => {
      ...
      function render(append = false) {
        ...
        if (paginationEl) {
          ...
          if (hasMore) {
            ...
            paginationEl.querySelector('.category-load-more').addEventListener('click', () => {
              currentPage += 1;
              render(true);
            });
          } else {
      ...
      if (subcatsEl) {
        subcatsEl.addEventListener('click', e => {
          ...
          e.preventDefault();
          subcatsEl.querySelectorAll('.ch-ref-subcat').forEach(el => el.classList.remove('active'));
    ...
    .catch(err => {
      ...
      grid.innerHTML = '<p class="category-empty">Не удалось загрузить товары. Попробуйте позже.</p>';
    });
}
